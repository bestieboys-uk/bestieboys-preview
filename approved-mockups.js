(() => {
  const sprite = 'assets/concepts/approved-genre-mockups-sprite.webp?v=approved3';
  const mockups = [
  ...
  function crop(className,index,alt){
  ...
  function install(){
    mockups.forEach(([base,index,alt])=>{
      ...
      if(summary?.tagName==='SUMMARY'&&!summary.querySelector('.approved-mockup-thumb')){
        const preview=crop('approved-mockup-thumb',index,alt);
        const quick=summary.querySelector('.quick');
        quick?summary.insertBefore(preview,quick):summary.appendChild(preview);
      }

      const body=concept.querySelector('.concept-body');
      if(body&&!body.querySelector('.approved-mockup-full')){
        body.insertBefore(crop('approved-mockup-full',index,alt),body.firstChild);
      }
